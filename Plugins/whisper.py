...
async def get_whisper(app,query):
  if query.data.endswith('+ar'):
    ...
    if r.get(id):
      ...
      if not 'all' in id and not str(query.from_user.id) in id and not query.from_user.id == 125205235:
    ...
    if "all" in id:
      ...
      if str(query.from_user.id) in id.split('+')[1]:
         ...
         try:
           ...
           pass
      if query.from_user.id == 125205235 or query.from_user.id ==125205235:
         return await query.answer(q[:200], show_alert=True, cache_time=600)
  else:
    id = query.data.split("whisper")[0]
    if r.get(id):
      ...
      await query.answer(
        ...
        results=[           
            InlineQueryResultArticle(
                ...
                thumb_width=128, thumb_height=128,
                description='@marilinbot Hello @h_uu_u',
